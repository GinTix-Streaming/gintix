  ...
  return (
    <div className="space-y-5">
      ...
      <div className="panel flex items-center justify-between p-5">
        <div>
          <p className="text-xs font-semibold uppercase tracking-wide text-ink-muted">Account balance</p>
          <p className="mt-1 text-xl font-extrabold text-ink">{money(ctx.advertiser.balance_cents)}</p>
        </div>
        <Link href="/advertise/dashboard/billing" className="btn-ghost">Add funds</Link>
      </div>

      {/* Campaigns */}
      <section className="panel overflow-hidden">
        <div className="flex items-center justify-between border-b border-white/5 px-5 py-4">
          <h2 className="font-bold text-ink">Campaigns</h2>
          <Link href="/advertise/dashboard/campaigns/new" className="text-sm font-semibold text-amethyst-soft hover:underline">
            + New campaign
          </Link>
        </div>
        ...
        {list.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-ink-muted">Launch your first campaign in under two minutes.</p>
                ...
                return (
                  <tr key={c.id} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
                    ...
                    <td className="px-5 py-3 text-ink-muted">
                      {c.total_budget_cents ? money(c.total_budget_cents) : c.daily_budget_cents ? money(c.daily_budget_cents) + "/day" : "—"}
                    </td>
                    <td className="px-5 py-3 text-ink-muted">{num(m.impressions)}</td>
                    <td className="px-5 py-3 text-ink-muted">{num(m.clicks)}</td>
